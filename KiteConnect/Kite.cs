...
namespace KiteConnect
{
    ...
    public class Kite 
    {
        ...
        //private string _root = "https://kite.zerodha.com";
        ...
        string MyAPIKey = "67uleb0ttdjmdp6v";
        string MySecret = "j7vwuykvh3n5ua2jt8dyd52v23814b7a";
        ...
        private readonly Dictionary<string, string> _routes = new Dictionary<string, string>
        {
            ...
            ["kite.login"]="/api/login",
            ["kite.twofa"]="/api/twofa",   
            ...
            //zerodhanew
            ["orders.placenew"] = "/oms/orders/regular",
            ["blank"]=""
        };
        public  Kite()
        {
        ...
        public void Login()
        {
            ...
            //lock (_sessionlock)
            {
                ...
                string loginURL = string.Format("{0}&redirect_url={1}", GetLoginURL(), redirectURL);
        ...
        private void initSession(string requestToken)
        {
            ...
            User user = GenerateSession(requestToken, MySecret);
        ...
        public  User GenerateSession(string RequestToken, string AppSecret)
        {
            ...
            //Console.WriteLine("api_token:" + _apiKey + ",Thread:" + Thread.CurrentThread.Name);
        ...
        /// <param name="ProductType">Margin product applied to the order (margin is blocked based on this)</param>
        /// <param name="OrderType">Order type (MARKET, LIMIT etc.)</param>
        /// <param name="Validity">Order validity</param>
        /// <param name="ExchangeInstrumentId">Exchange instrumentid(IIFL)</param>
        ...
        public Dictionary<string, dynamic> PlaceOrder(string Exchange,
               int Quantity, string TradingSymbol = null, string TransactionType = null,
                decimal? Price = null, string ProductType = null, string OrderType = null, string Validity = null,
                string ExchangeInstrumentId = null, int? DisclosedQuantity = null, decimal? TriggerPrice = null,
                decimal? SquareOffValue = null, decimal? StoplossValue = null, string Variety = "", string Tag = "",string UserId="")
        {
            var param = new Dictionary<string, dynamic>();
            ...
            Utils.AddIfNotNull(param, "product", ProductType);
            ...
            Utils.AddIfNotNull(param, "user_id", UserId);
            Utils.AddIfNotNull(param, "tag", Tag);

            return Post("orders.placenew", param);
        ...
        /// <returns></returns>
        public Dictionary<string, dynamic> LoginToKiteWithPin(
            string LoginId,
            string Password,
            string pin)
        {
            ...
            Utils.AddIfNotNull(param, "user_id", LoginId);
            Utils.AddIfNotNull(param, "password", Password);
            //dynamic getcookies = Get("blank");
            Dictionary<string,dynamic> dictresult= Post("kite.login", param);
            Dictionary<string, dynamic> dict1 = dictresult["data"];
            string requestid = dict1["request_id"];
            string cfduid = dictresult["cfduid"];
            string kfsession = dictresult["kfsession"];

            param.Clear();
            //2FA
            Utils.AddIfNotNull(param, "user_id", LoginId);
            Utils.AddIfNotNull(param, "request_id", requestid);
            Utils.AddIfNotNull(param, "twofa_value", pin);
            Utils.AddIfNotNull(param, "cfduid", cfduid);
            Utils.AddIfNotNull(param, "kfsession", kfsession);
            return Post("kite.twofa", param);
        ...
        {
            return Request1(Route, "GET", Params);
        }
        ...
        {
            return Request1(Route, "POST", Params);
        }

        //private dynamic Post1(string Route, Dictionary<string, dynamic> Params = null)
        //{
        ...
        private void AddExtraHeaders(ref HttpWebRequest Req)
        {
            ...
            //Req.Headers.Add("Cookies", "__cfduid=d79531434a9341be210d01c89f0c0ad661602698705");
            if (_acccessTokenCfduid != "")
            {
                Cookie cookie = new Cookie();
                cookie.Name = "__cfduid"; cookie.Value = _acccessTokenCfduid;
                cookiecoll.Add(cookie);
                //Req.CookieContainer.Add("__cfduid", AccessTokenCfduid);
                //Req.Headers.Add("Cookie", "__cfduid=" + AccessTokenCfduid);
            }
            if(_accessTokenkfsession != "")
            {
                Cookie cookie = new Cookie();
                cookie.Name = "kf_session"; cookie.Value = _accessTokenkfsession;
                cookiecoll.Add(cookie);
                //Req.Headers.Add("Cookie", "kf_session=" + AccessTokenkfsession);
            }
            if (cookiecoll.Count > 0)
            {
               // Req.CookieContainer = new CookieContainer();
                CookieContainer gaCookies = new CookieContainer();
                Uri target = new Uri("http://kite.zerodha.com/");

                gaCookies.Add(new Cookie("__cfduid", _acccessTokenCfduid) { Domain = target.Host });
                gaCookies.Add(new Cookie("kf_session", _accessTokenkfsession) { Domain = target.Host });
            ...
            //{
            //    Req.Headers.Add("Authorization", "enctoken " + _accessToken);
            //}
        ...
        private string _acccessTokenCfduid = "";
        private string _accessTokenkfsession = "";
        private string _accessTokenenc = "";
        private dynamic Request1(string Route, string Method, Dictionary<string, dynamic> Params = null)
        {
            string url = _root + _routes[Route];
            //extract __cfduid
            dynamic cfdval = "";
            dynamic kfsession = "";
            if(Params.TryGetValue("cfduid",out cfdval))
            {
                _acccessTokenCfduid = cfdval;
            }
            if (Params.TryGetValue("kfsession", out kfsession))
            {
                _accessTokenkfsession = kfsession;
            }
            ...
            try
            {
                ...
                //get kf_session
                int index = cookies.IndexOf("kf_session=");
                if (index >= 0)
                {
                    int indexsem = cookies.IndexOf(";", index);
                    kf_session = cookies.Substring(index + 11, (indexsem - (index + 11)));
                }

                int indexcfduid = cookies.IndexOf("__cfduid=");
                if (indexcfduid > 0)
                {
                    int indexcfduidsem = cookies.IndexOf(";", indexcfduid);
                    cfduid_session = cookies.Substring(indexcfduid + 9, (indexcfduidsem - (indexcfduid + 9)));
                }

                int indexenc = cookies.IndexOf("enctoken=");
                if(indexenc>=0)
                {
                    int indexencsem = cookies.IndexOf(";", indexenc);
                    enc_token = cookies.Substring(indexenc + 9, (indexencsem - (indexenc + 9)));
            ...
            using (Stream webStream = webResponse.GetResponseStream())
            {
                using (StreamReader responseReader = new StreamReader(webStream))
                {
                    ...
                    {
                        Dictionary<string, dynamic> responseDictionary = Utils.JsonDeserialize(response);
                        responseDictionary.Add("cfduid", cfduid_session);
                        responseDictionary.Add("kfsession", kf_session);
                        responseDictionary.Add("enctoken", enc_token);
